<x-layout>
    ...
    <div class="container-fluid">
        ...
        <p>Categorie:</p>
        <ul>
            @forelse ($boardgame->categories as $category)
                <li>{{ $category->name }}</li>
            @empty
                <li> Nessuna Categoria selezionata</li>
            @endforelse
        </ul>
                ...
                <div class="card mb-4" style="width: 18rem;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="text-center">
                            <a href="{{ route('boardgame.edit', compact('boardgame')) }}"
                                class="btn btn-primary">Modifica Scheda</a>
                            <form action="{{ route('boardgame.delete', compact('boardgame')) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-danger"
                                    onclick="confirm('Sei sicuro di voler eliminare la scheda?')" type="submit">Elimina
                                    Scheda</button>
                            </form>
                        </div>
                    </div>
